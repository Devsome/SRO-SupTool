...
namespace VEGA.Framework.Bot.FileHandling
{
    public class TXT
    {

        #region "HashTable 1"
        private static byte[] Hash_Table_1 =
        {
        ...
        #endregion
        #region "HashTable 2"
        private static byte[] Hash_Table_2 =
        {
        ...
        public static byte[] DecryptEncryptedTextfile(byte[] textfile)
        {
            bool encrypted = false;
            uint key = 0x8C1F;
            sbyte buff;

            if (textfile[0] == 0xE2 && textfile[1] == 0xB0)
                encrypted = true;

            try
            {
                for (uint i = 0; i < (uint)textfile.Length - 26; i++)
                {
                    buff = (sbyte)(Hash_Table_1[key % 0xA7] - Hash_Table_2[key % 0x1EF]);
                    ++key;

                    if (encrypted)
                        textfile[i] += (byte)buff;
                    else
                        textfile[i] -= (byte)buff;
                }
            }
